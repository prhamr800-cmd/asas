...
export default function App() {
  ...
  // Register Android Back Button handler and Deep Link listener
  useEffect(() => {
    const unregister = androidBridge.registerBackHandler(() => {
      if (showGeminiLive) { setShowGeminiLive(false); return true; }
      if (showSubscriptionModal) { setShowSubscriptionModal(false); return true; }
      ...
      if (selectedProfileUserId) { setSelectedProfileUserId(null); return true; }
      if (selectedProfileGroupId) { setSelectedProfileGroupId(null); return true; }
      if (showAdvancedSearch) { setShowAdvancedSearch(false); return true; }
      if (showAiAssistant) { setShowAiAssistant(false); return true; }
    ...
    const handleDeepLink = (e: any) => {
      ...
      if (url.includes("chat/")) {
        const id = url.split("chat/")[1]?.split("?")[0]?.split("/")[0];
        if (id) setActiveChatId(id);
      } else if (url.includes("user/")) {
        const id = url.split("user/")[1]?.split("?")[0]?.split("/")[0];
        if (id) setSelectedProfileUserId(id);
      } else if (url.includes("group/")) {
        const id = url.split("group/")[1]?.split("?")[0]?.split("/")[0];
        if (id) setSelectedProfileGroupId(id);
      }
    };
    window.addEventListener("privo-deep-link", handleDeepLink);

    return () => {
      unregister();
      window.removeEventListener("privo-deep-link", handleDeepLink);
    };
  }, [
    showGeminiLive, showSubscriptionModal, showAdminDashboard, showSettings,
    showNewChat, showGifPicker, showEmojiPicker, selectedProfileUserId,
    selectedProfileGroupId, showAdvancedSearch, showAiAssistant, activeChatId
  ]);
